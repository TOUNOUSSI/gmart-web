...
export class AuthService {
  ...
  public logIn(user: User) {
    localStorage.clear();
    return this.https
      .post(API_GATEWAY_URL + CORE_ACCOUNTS_URN + "/auth/signin", user)
      .pipe(
        tap((response: any) => {
          if (response !== undefined) {
            console.log("pseudoname: " + JSON.stringify(response));
  ...
  signup(user: User) {
    return this.https
      .post(API_GATEWAY_URL + CORE_ACCOUNTS_URN + "/signup", user)
      .toPromise()
  ...
  isLoggedInToWebservice(): boolean {
    try {
      if (this.getToken() === undefined || this.getToken() === "") {
        ...
        this.https.get(API_GATEWAY_URL + "/account/isLoggedIn").subscribe(
          (response: any) => {
  ...
  logout() {
    return this.https
      .get(API_GATEWAY_URL + AUTH_SERVER_URN + "/logout")
      .toPromise()
      .then((response: Response) => response)
      .catch((error) => {
        this.router.navigate(["/signin"]);
