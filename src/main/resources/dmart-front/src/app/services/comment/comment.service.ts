...
export class CommentService {
  ...
  getAllRecentPosts(): Observable<any> {
    return this.https
      .get<Comment[]>(API_GATEWAY_URL + CORE_COMMENT_URN + "/all-recent-posts")
      .pipe(
  ...
  addNewCommentOnPost(comment: Comment, postID: string) {
    return this.https
      .post(
        API_GATEWAY_URL +
          CORE_COMMENT_URN +
      ...
      .pipe(
        tap((response: any) => {
          this.toasterService.message =
            "Nouveau commentaire a été ajouté avec succèes";
